...
export default function MySavedContainer({ userId }: { userId: string }) {
  const { data, isLoading } = trpc.getSavedBlogs.useQuery({ userId });
  if (isLoading) {
    return (
  ...
  return (
    <div className="w-full flex flex-col gap-3">
      {data?.map((item) => (
        <LongBlogCard key={item.id} item={item} isSaved />
      ))}
    </div>
  );
}
